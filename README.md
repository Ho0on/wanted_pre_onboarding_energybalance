...
## 🤖 검색 알고리즘 설명
- 사용자의 입력값과 불러온 데이터 모두 같은 필터링 함수를 통해 가공시킴
- 하나는 띄어쓰기를 없애는 기능이고 또 하나는 데이터가 영어일때 모두 소문자로 바꿔 통일시키는 필터링 함수
- 주어진 데이터에는 제품명, 브랜드명 이 두가지 컬럼밖에 존재하지 않았기 때문에 빠른 검색을 위해 입력값과 데이터 모두 배열로 비교하지 않고 문자열 대 문자열로 비교하였음
- 브랜드명이 있는 상품들을 우선순위로 하여 목록의 상단에 위치하도록 배치
- 브랜드명이 명확한 제품이 신뢰도가 높다고 판단하여 소비자에게 우선적으로 노출시키기 위해 순위를 높임
